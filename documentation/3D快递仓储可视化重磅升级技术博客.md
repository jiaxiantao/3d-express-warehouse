...
> 上一篇基础篇：[3D快递仓储可视化技术博客](./3D快递仓储可视化技术博客.md)
...
如果你已经读过 [基础技术博客](./3D快递仓储可视化技术博客.md)，本文重点讲 **v1.0 之后新增了什么、难在哪里、下一步往哪扩**。

---

## 效果预览

![重磅升级全貌（截图占位）](images/blog-v2-01-dashboard.png)

> **截图占位**：打开 `/warehouse`，默认上帝视角，截取含货架、栅栏、机器人、底栏视角切换与右侧方向键的完整画面，保存为 `documentation/images/blog-v2-01-dashboard.png`。
...
flowchart TB
  subgraph ui [页面层]
    ...
    Pad["warehouse-robot-pad"]
    Scan["warehouse-qr-scanner"]
    Toggle["warehouse-perspective-toggle"]
  end
  subgraph scene [3D 场景层]
    Scene["warehouse-scene.tsx"]
    Robot["warehouse-robot.tsx"]
    Nav["warehouse-robot-floor-nav"]
    Labels["warehouse-rack-category-labels"]
    Fence["warehouse-fence.tsx"]
  end
  subgraph lib [逻辑层]
    Drive["warehouse-robot-drive.ts"]
    Motion["warehouse-robot-motion.ts"]
    Path["warehouse-robot-navigation.ts"]
    Category["warehouse-rack-category.ts"]
    ScanLib["warehouse-scan.ts"]
    Data["warehouse-data.ts"]
  end
  Page --> Scene
  Page --> Pad
  Page --> Scan
  Pad -->|setRobotDrive| Scene
  Scene --> Robot
  Scene --> Nav
  Scene --> Labels
  Scene --> Fence
...
![机器人第三人称（截图占位）](images/blog-v2-02-robot-third.png)

> **截图占位**：第三人称视角下机器人站在巷道中间，保存为 `documentation/images/blog-v2-02-robot-third.png`。
...
![三种视角对比（截图占位）](images/blog-v2-03-camera-modes.png)

> **截图占位**：分别截取上帝 / 第三 / 第一人称同一场景，拼成横向对比图，保存为 `documentation/images/blog-v2-03-camera-modes.png`。
...
![货柜分类标签（截图占位）](images/blog-v2-04-rack-labels.png)

> **截图占位**：上帝视角特写三列货柜端头分类牌，保存为 `documentation/images/blog-v2-04-rack-labels.png`。
...
![扫码定位（截图占位）](images/blog-v2-05-qr-scan.png)

> **截图占位**：打开扫码弹窗或展示 `?sku=FB-1001` 定位成功后的选中效果，保存为 `documentation/images/blog-v2-05-qr-scan.png`。
...
- [3D快递仓储可视化技术博客](./3D快递仓储可视化技术博客.md) — v1.0 基础篇（InstancedMesh、筛选、描边）  
